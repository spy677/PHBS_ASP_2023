...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        prices = forward[:,None]*np.exp(chol_m @ znorm_m*np.sqrt(texp) - np.diagonal(cov_m)[:,None]*texp/2)
...
def basket_price_norm_analytic(
    ...
    newforward=forward @weights
    cov_m=vol * cor_m * vol[:,None]
    newvolatility=weights @cov_m@ weights[:,None]
    norm=pf.Norm(sigma=np.sqrt(newvolatility))
    price=norm.price(strike, newforward, texp,cp)
